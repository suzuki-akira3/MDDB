...
from python.extraction.rdf_base_functions import RDFBaseFunc
from python.annotation.phrase_tools import CheckBaseWord
from python.parameters.common_parameters import CommonParameters
from python.modules.dictionary import append_dictionary
        ...
        for tupl in self.basephrases:
            ...
            for label in labels:
                if label[1:-1] in clas:
                    ...
                    # print([( '-'.join((label[1:-1], baseword_base)), tupl[0] ) ]  )
...
#                                     
            ...
            for label in labels:
                if label[1:-1] in clas:
                    ...
                    # print([( '-'.join((label[1:-1], baseword_first)), tupl[0] ) ])
                    if self.phrase != tupl[0]:
                        phrases += [(tupl[0], self.phrase)]               
...
class Convert2RDF(RDFBaseFunc):
    attribs = ['Annotation_label', 'Relation', 'basephrase_label', 'basephrase_relation', 'baseword', 'basephrase', 'phrase']
